    ...
    <section>
      <article class="bright">
          <?php
            ...
            $query = "SELECT idOrden, total, formaDePago, fecha, direccion, isCompleted FROM ordenes WHERE idUsuario=$userId ORDER BY fecha ASC;";
          ...
          <?php
            if(mysqli_num_rows($data) == 0) {
              ...
              foreach($prod as $row) {
                ...
                echo($msg.'</p>');
                echo('<button onclick="location.href=\'showOrder.php?orderid='.$row['idOrden'].'\'">Ver orden</button>');
                echo('</div>');
                echo('</div>');
              }
            }
